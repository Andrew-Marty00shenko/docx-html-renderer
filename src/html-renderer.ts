...
export class HtmlRenderer {
			...
			for (let ref of f.embedFontRefs) {
				this.tasks.push(
					this.document.loadFont(ref.id, ref.key).then((fontData) => {
						...
						const cssText = this.styleToString(
							"@font-face",
							cssValues
						);
						styleContainer.appendChild(
							this.createComment(`docxjs ${f.name} font`)
						);
						styleContainer.appendChild(
	...
	splitBySection(
		elements: OpenXmlElement[],
		defaultProps: SectionProperties
	): Section[] {
		var current: Section = {
			sectProps: null,
			elements: [],
...
`;
	...
	renderTableRow(elem: WmlTableRow) {
		...
		if (elem.gridBefore)
			result.appendChild(
				this.renderTableCellPlaceholder(elem.gridBefore)
			);
	...
	renderVmlChildElement(elem: VmlElement): any {
		...
		for (let child of elem.children) {
			if (child.type == DomType.VmlElement) {
				result.appendChild(
					this.renderVmlChildElement(child as VmlElement)
				);
			} else {
				result.appendChild(
					...asArray(this.renderElement(child as any))
				);
			}
		}
	...
	renderMmlDelimiter(elem: OpenXmlElement): HTMLElement {
		...
		children.push(
			this.createElementNS(ns.mathML, "mo", null, [
	...
	renderMmlNary(elem: OpenXmlElement): HTMLElement {
		...
		const charElem = this.createElementNS(ns.mathML, "mo", null, [
			elem.props?.char ?? "\u222B",
		]);

		if (supElem || subElem) {
			children.push(
				this.createElementNS(ns.mathML, "munderover", null, [
					charElem,
					subElem,
					supElem,
				])
			...
			children.push(
				this.createElementNS(ns.mathML, "mover", null, [
					charElem,
					supElem,
				])
			);
			...
			children.push(
				this.createElementNS(ns.mathML, "munder", null, [
					charElem,
					subElem,
				])
			);
	...
	renderMmlPreSubSuper(elem: OpenXmlElement) {
		...
		children.push(
			this.createElementNS(ns.mathML, "msubsup", null, [
				stubElem,
				subElem,
				supElem,
			])
	...
	renderMmlGroupChar(elem: OpenXmlElement) {
		...
		if (elem.props.char) {
			result.appendChild(
				this.createElementNS(ns.mathML, "mo", null, [elem.props.char])
			);
		}
